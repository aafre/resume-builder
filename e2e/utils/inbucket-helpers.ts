/**
 * Inbucket Email Helpers for E2E Testing
 *
 * Inbucket is a local email testing server bundled with Supabase.
 ...
 *
 * Inbucket runs on port 54324 (check with: supabase status)
 * Web UI: http://127.0.0.1:54324
 * API: http://127.0.0.1:54324/api/v1
 */

const INBUCKET_API_URL = process.env.INBUCKET_API_URL || 'http://127.0.0.1:54324/api/v1';

/**
 * Inbucket message metadata
 */
...
/**
 * Full Inbucket message with content
 */
...
/**
 * Wait for email to arrive in Inbucket inbox
 *
 * Polls Inbucket API every 500ms for up to 30 seconds by default.
...
export async function waitForEmail(
  ...
  const mailbox = recipientEmail.split('@')[0]; // Extract mailbox name from email
  ...
  while (Date.now() - startTime < timeout) {
    try {
      // Fetch messages from Inbucket API
      const response = await fetch(`${INBUCKET_API_URL}/mailbox/${mailbox}`);

      if (!response.ok) {
        throw new Error(`Inbucket API error: ${response.status} ${response.statusText}`);
      }

      const messages: InbucketMessage[] = await response.json();

      // Filter by subject if specified
      let matchingMessage = messages[0]; // Latest message

      if (subjectFilter && messages.length > 0) {
        matchingMessage = messages.find(m =>
          m.subject.toLowerCase().includes(subjectFilter.toLowerCase())
        ) || null;
      }

      if (matchingMessage) {
        console.log(`✅ Email received: "${matchingMessage.subject}"`);
        console.log(`   From: ${matchingMessage.from}`);
        console.log(`   ID: ${matchingMessage.id}`);
      ...
      console.error(`⚠️  Error polling Inbucket:`, error);
...
/**
 * Fetch full email content from Inbucket
 *
 * Retrieves the complete email including HTML, text, and headers.
 *
 * @param recipientEmail - Email address (e.g., "e2e-test@example.com")
...
export async function getEmailContent(
  ...
  const response = await fetch(`${INBUCKET_API_URL}/mailbox/${mailbox}/${messageId}`);

  if (!response.ok) {
    throw new Error(
      `Failed to fetch email content: ${response.status} ${response.statusText}\n` +
      `Mailbox: ${mailbox}, Message ID: ${messageId}`
    );
  }

  const content: InbucketMessageDetail = await response.json();
  console.log(`✅ Email content fetched (${content.html.length} chars HTML)`);
