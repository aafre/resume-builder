...
_MARKDOWN_LINK_PATTERN_LATEX = re.compile(r"\[([^\]]+)\]\(([^\)]+)\)")
...
def convert_markdown_links_to_latex(text):
    ...
    latex_text = _MARKDOWN_LINK_PATTERN_LATEX.sub(r"\\href{\2}{\1}", text)

    return latex_text


_MD_BOLD_1_LATEX = re.compile(r"\*\*(.+?)\*\*")
_MD_BOLD_2_LATEX = re.compile(r"__(.+?)__")
_MD_ITALIC_1_LATEX = re.compile(r"\*(.+?)\*")
_MD_ITALIC_2_LATEX = re.compile(r"_(.+?)_")
_MD_STRIKE_LATEX = re.compile(r"~~(.+?)~~")
_MD_UNDERLINE_LATEX = re.compile(r"\+\+(.+?)\+\+")
...
def convert_markdown_formatting_to_latex(text):
    ...
    text = _MD_BOLD_1_LATEX.sub(r"\\textbf{\1}", text)

    # 2. Bold with __ (must come before single _)
    text = _MD_BOLD_2_LATEX.sub(r"\\textbf{\1}", text)

    # 3. Italic with * (after ** is processed)
    text = _MD_ITALIC_1_LATEX.sub(r"\\textit{\1}", text)

    # 4. Italic with _ (after __ is processed)
    text = _MD_ITALIC_2_LATEX.sub(r"\\textit{\1}", text)

    # 5. Strikethrough with ~~
    text = _MD_STRIKE_LATEX.sub(r"\\sout{\1}", text)

    # 6. Underline with ++ (custom syntax)
    text = _MD_UNDERLINE_LATEX.sub(r"\\underline{\1}", text)
...
_LATEX_ESC_US = re.compile(r"(?<!\\)_")
_LATEX_ESC_TILDE = re.compile(r"(?<!\\)~")
...
def _escape_remaining_latex_chars(text):
    ...
    text = _LATEX_ESC_US.sub(r"\\_", text)

    # Escape tildes NOT already escaped
    text = _LATEX_ESC_TILDE.sub(r"\\textasciitilde{}", text)
