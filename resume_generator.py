...
def convert_markdown_links_to_html(text):
    ...
    html_text = _MARKDOWN_LINK_PATTERN.sub(r'<a href="\2">\1</a>', text)

    return html_text


_MD_BOLD_1 = re.compile(r"\*\*(.+?)\*\*")
_MD_BOLD_2 = re.compile(r"__(.+?)__")
_MD_ITALIC_1 = re.compile(r"\*(.+?)\*")
_MD_ITALIC_2 = re.compile(r"_(.+?)_")
_MD_STRIKE = re.compile(r"~~(.+?)~~")
_MD_UNDERLINE = re.compile(r"\+\+(.+?)\+\+")
...
def convert_markdown_formatting_to_html(text):
    ...
    text = _MD_BOLD_1.sub(r"<strong>\1</strong>", text)

    # 2. Bold with __ (must come before single _)
    text = _MD_BOLD_2.sub(r"<strong>\1</strong>", text)

    # 3. Italic with * (after ** is processed)
    text = _MD_ITALIC_1.sub(r"<em>\1</em>", text)

    # 4. Italic with _ (after __ is processed)
    text = _MD_ITALIC_2.sub(r"<em>\1</em>", text)
