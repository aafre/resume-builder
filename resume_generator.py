...
_MD_BOLD_STAR_PATTERN = re.compile(r"\*\*(.+?)\*\*")
_MD_BOLD_UNDERSCORE_PATTERN = re.compile(r"__(.+?)__")
_MD_ITALIC_STAR_PATTERN = re.compile(r"\*(.+?)\*")
_MD_ITALIC_UNDERSCORE_PATTERN = re.compile(r"_(.+?)_")
_MD_STRIKETHROUGH_PATTERN = re.compile(r"~~(.+?)~~")
_MD_UNDERLINE_PATTERN = re.compile(r"\+\+(.+?)\+\+")
...
def convert_markdown_formatting_to_html(text):
    ...
    text = _MD_BOLD_STAR_PATTERN.sub(r"<strong>\1</strong>", text)

    # 2. Bold with __ (must come before single _)
    text = _MD_BOLD_UNDERSCORE_PATTERN.sub(r"<strong>\1</strong>", text)

    # 3. Italic with * (after ** is processed)
    text = _MD_ITALIC_STAR_PATTERN.sub(r"<em>\1</em>", text)

    # 4. Italic with _ (after __ is processed)
    text = _MD_ITALIC_UNDERSCORE_PATTERN.sub(r"<em>\1</em>", text)

    # 5. Strikethrough with ~~
    text = _MD_STRIKETHROUGH_PATTERN.sub(r"<s>\1</s>", text)

    # 6. Underline with ++ (custom syntax)
    text = _MD_UNDERLINE_PATTERN.sub(r"<u>\1</u>", text)
...
_SO_URL_PATTERN = re.compile(r"/users/\d+/([\w\-]+)")
_HANDLE_NUMBER_PATTERN = re.compile(r"\d{4,}")
_HANDLE_RANDOM_SUFFIX_PATTERN = re.compile(r"-[a-z0-9]{8,}")
...
def get_social_media_handle(url, platform="linkedin"):
    ...
    if platform == "stackoverflow":
        ...
        match = _SO_URL_PATTERN.search(url)
...
def generate_linkedin_display_text(linkedin_url, contact_name=None):
    ...
    def is_clean_handle(handle):
        ...
        if _HANDLE_NUMBER_PATTERN.search(handle):
            return False

        # Rule 4: Common random suffixes (e.g., ...-a1b2c3d4)
        if _HANDLE_RANDOM_SUFFIX_PATTERN.search(handle.lower()):
            return False
