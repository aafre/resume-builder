...
- **To actually change prod robots.txt:** update the GCS bucket object (owner/infra with account access): `gsutil -h "Content-Type:text/plain" cp robots.txt gs://easyfreeresume-static/robots/robots.txt`.
  - **Gotcha 1 — ACL:** `gsutil cp` overwrites the object as **private**, so anonymous reads then 403 (which would take `/robots.txt` down site-wide once the edge cache lapses). Re-apply public read every time: `gsutil acl ch -u AllUsers:R gs://easyfreeresume-static/robots/robots.txt`.
