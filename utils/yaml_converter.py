...
def fast_yaml_dump(data, stream=None, **kwargs):
    ...
    """
    # ⚡ Bolt Optimization: Using SafeDumper (CSafeDumper) avoids Python-level overhead and provides ~10x speedup.
...
def json_to_yaml_structure(resume_data: Dict[str, Any]) -> str:
    ...
    yaml_string = fast_yaml_dump(
        ...
        allow_unicode=True,
        sort_keys=False,
        width=2147483646,  # Prevent line wrapping; float("inf") causes OverflowError in CSafeDumper
    )
