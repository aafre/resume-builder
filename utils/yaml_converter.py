...
# Attempt to load PyYAML's CSafeLoader for high-performance parsing.
# Evaluated once at the module level to avoid overhead on subsequent calls.
try:
...
def fast_yaml_load(stream):
    """
    High-performance YAML loading utility using PyYAML's CSafeLoader if available.
    Falls back to SafeLoader if C-extensions are missing.
    Provides ~10x parsing speedup on large configurations or I/O bound tasks.
    """
