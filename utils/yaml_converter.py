...
def fast_yaml_dump(data, stream=None, **kwargs):
    ...
    Uses C-based CSafeDumper if available (~3-4x speedup).
    Note: CSafeDumper cannot handle width=float('inf'), use a large int instead.
    """
...
def json_to_yaml_structure(resume_data: Dict[str, Any]) -> str:
    ...
    yaml_string = fast_yaml_dump(
        ...
        width=int(
            1e9
        ),  # Prevent line wrapping (CSafeDumper raises OverflowError on float('inf'))
    )
