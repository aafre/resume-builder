...
$VITE_AFFILIATE_JOB_SEARCH_ENABLED = if ($env:VITE_AFFILIATE_JOB_SEARCH_ENABLED) { $env:VITE_AFFILIATE_JOB_SEARCH_ENABLED } else { "true" }
...
$VITE_POSTHOG_HOST = if ($env:VITE_POSTHOG_HOST) { $env:VITE_POSTHOG_HOST } else { "" }
$VITE_TURNSTILE_SITE_KEY = if ($env:VITE_TURNSTILE_SITE_KEY) { $env:VITE_TURNSTILE_SITE_KEY } else { "" }
...
if ($VITE_POSTHOG_KEY) { Write-Host "  VITE_POSTHOG_KEY: set" } else { Write-Host "  VITE_POSTHOG_KEY: (unset - analytics disabled)" }
if ($VITE_TURNSTILE_SITE_KEY) { Write-Host "  VITE_TURNSTILE_SITE_KEY: set" } else { Write-Host "  VITE_TURNSTILE_SITE_KEY: (unset - Turnstile skipped)" }
...
Write-Host "  (leave JOBS_PSEO_ENABLED unset: job SEO pages stay off, see ADR-0001)"
