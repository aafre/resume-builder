...
class TestSaveResumeBasics:
    ...
    def test_save_resume_updates_existing(
...
class TestSmartDiffing:
    ...
    def test_save_resume_skips_when_hash_unchanged(
        ...
        json_repr = json.dumps(
            {"contact_info": contact_info, "sections": sections, "icon_metadata": []},
            sort_keys=True,
        )
        ...
        mock_sb.table.return_value.execute.side_effect = [
            create_mock_response(
                [{"id": TEST_RESUME_ID, "json_hash": expected_hash}]
            ),  # verify ownership
    ...
    def test_save_resume_proceeds_when_hash_changed(
        self, flask_test_client, auth_headers
    ):
        ...
        mock_sb.table.return_value.execute.side_effect = [
            create_mock_response(
                [{"id": TEST_RESUME_ID, "json_hash": "different-hash"}]
            ),  # verify ownership
            create_mock_response([]),  # get existing icons
            create_mock_response([]),  # upsert resume
            create_mock_response([]),  # delete icons
...
class TestSaveResumeValidation:
    ...
    def test_save_resume_requires_json_data(self, flask_test_client, auth_headers):
        ...
        response = client.post(
            "/api/resumes", json={}, headers=auth_headers  # Send empty JSON object
        )
    ...
    def test_save_resume_returns_404_for_nonexistent(
        self, flask_test_client, auth_headers
    ):
...
class TestLinkedInMigration:
    ...
    def test_save_resume_migrates_linkedin_to_social_links(
        self, flask_test_client, auth_headers
    ):
    ...
    def test_migrate_linkedin_handles_empty_linkedin(self, flask_test_client):
        ...
        contact_info = {"name": "John Doe", "linkedin": ""}  # Empty
...
class TestSaveResumeAIMetadata:
    ...
    def test_save_resume_stores_ai_import_warnings(
        self, flask_test_client, auth_headers
    ):
