...
class TestUploadIconToStorage:
    ...
    def test_upload_icon_returns_public_url(self, flask_test_client):
        ...
        expected_url = (
            f"https://test.supabase.co/storage/{TEST_USER_ID}/{TEST_RESUME_ID}/icon.png"
        )
...
class TestDownloadIconFromStorage:
    ...
    def test_download_icon_writes_to_destination(
    ...
    def test_download_icon_retries_on_transient_error(
        self, flask_test_client, temp_output_dir
    ):
        ...
        success = flask_app.download_icon_from_storage(
            storage_path, str(dest_path), max_retries=2
        )

        assert success is True
        assert mock_sb.storage.from_.return_value.download.call_count == 2

    def test_download_icon_returns_false_after_max_retries(
        self, flask_test_client, temp_output_dir
    ):
    ...
    def test_download_icon_with_supabase_none_returns_false(
        self, flask_test_client, temp_output_dir
    ):
...
class TestSaveResumeWithIcons:
    ...
    def test_save_resume_uploads_new_icons(self, flask_test_client, auth_headers):
        ...
        response = client.post(
            ...
            json={
                ...
                "sections": [],
                "icons": [{"filename": "new_icon.png", "data": icon_data}],
            },
    ...
    def test_save_resume_preserves_unchanged_icons(
        self, flask_test_client, auth_headers, sample_icon_data
    ):
    ...
    def test_save_resume_deletes_removed_icons(
        self, flask_test_client, auth_headers, sample_icon_data
    ):
