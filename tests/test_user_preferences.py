...
class TestGetUserPreferences:
    ...
    def test_get_preferences_returns_defaults_if_none(
    ...
    def test_get_preferences_creates_default_on_first_access(
        self, flask_test_client, auth_headers
    ):
...
class TestUpdateUserPreferences:
    ...
    def test_update_preferences_requires_auth(self, flask_test_client):
        ...
        response = client.post(
            "/api/user/preferences", json={"last_edited_resume_id": TEST_RESUME_ID}
        )
...
class TestPreferencesEdgeCases:
    ...
    def test_get_preferences_handles_database_error(
        self, flask_test_client, auth_headers
    ):
    ...
    def test_update_preferences_handles_database_error(
        self, flask_test_client, auth_headers
    ):
        ...
        # Configure mock to raise error
        mock_sb.table.return_value.upsert.return_value.execute.side_effect = Exception(
            "Database error"
        )
    ...
    def test_preferences_with_empty_preferences_object(
        self, flask_test_client, auth_headers
    ):
        ...
        response = client.post(
            "/api/user/preferences",
            json={"last_edited_resume_id": None, "preferences": {}},
            headers=auth_headers,
        )

        assert response.status_code == 200

    def test_preferences_preserves_nested_objects(
        self, flask_test_client, auth_headers
    ):
        ...
        response = client.post(
            "/api/user/preferences",
            json={"last_edited_resume_id": TEST_RESUME_ID, "preferences": nested_prefs},
            headers=auth_headers,
        )
