...
describe('useContactForm', () => {
  ...
  describe('handleRemoveSocialLink', () => {
    ...
    it('should remap error indices when removing a link before the error', () => {
      // Setup: 3 links where link at index 1 has an error
      contactInfo.social_links = [
        { platform: 'github', url: 'https://github.com/user0', display_text: '' },
        { platform: 'linkedin', url: '', display_text: '' },
        { platform: 'twitter', url: '', display_text: '' },
      ];
      ...
      // Trigger validation error at index 1 by setting invalid URL
      act(() => {
        result.current.handleSocialLinkChange(
          1,
          'url',
      ...
      // Verify error exists at index 1
      expect(result.current.socialLinkErrors[1]).toBeDefined();
      expect(result.current.socialLinkErrors[1]).toBe('Invalid URL for platform');

      // Remove link at index 0
      act(() => {
        result.current.handleRemoveSocialLink(0);
      });

      // Verify error shifted from index 1 to index 0
      expect(result.current.socialLinkErrors[0]).toBe('Invalid URL for platform');
      expect(result.current.socialLinkErrors[1]).toBeUndefined();
    });

    it('should remap auto-generated indices when removing a link before them', () => {
      // Setup: 3 links
      contactInfo.social_links = [
        { platform: 'twitter', url: '', display_text: '' },
        { platform: 'linkedin', url: '', display_text: '' },
      ...
      // Trigger auto-generation at index 2
      act(() => {
        result.current.handleSocialLinkChange(
          2,
          'url',
      ...
      // Verify auto-generated flag exists at index 2
      ...
      // Verify auto-gen flag shifted from index 2 to index 1
      expect(result.current.autoGeneratedIndexes.has(1)).toBe(true);
