...
describe('useContactForm', () => {
  ...
  describe('handleRemoveSocialLink', () => {
    ...
    it('should remap indices when removing a social link from the middle', () => {
      contactInfo.social_links = [
        { platform: 'github', url: 'https://github.com/user1', display_text: 'GitHub' },
        { platform: 'linkedin', url: 'https://linkedin.com/in/user2', display_text: 'LinkedIn' },
        { platform: 'twitter', url: 'https://twitter.com/user3', display_text: 'Twitter' },
      ];
      ...
      // Set up state for index 1 (error) and index 2 (auto-generated flag)
      act(() => {
        result.current.handleSocialLinkChange(1, 'url', 'invalid-url');
        result.current.handleSocialLinkChange(2, 'url', 'https://twitter.com/user3');
      });
      ...
      expect(result.current.socialLinkErrors[1]).toBe('Invalid URL for platform');
      expect(result.current.autoGeneratedIndexes.has(2)).toBe(true);

      // Remove link at index 0 - this should shift indices 1 and 2 down
      act(() => {
        result.current.handleRemoveSocialLink(0);
      });

      // Verify indices were remapped:
      // - Error that was on index 1 should now be on index 0
      // - Auto-gen flag that was on index 2 should now be on index 1
      ...
      expect(result.current.autoGeneratedIndexes.has(1)).toBe(true);
      expect(result.current.autoGeneratedIndexes.has(2)).toBe(false);
    });

    it('should preserve state for items before removed index', () => {
      contactInfo.social_links = [
        { platform: 'github', url: 'https://github.com/user1', display_text: 'GitHub' },
        { platform: 'linkedin', url: 'invalid', display_text: 'LinkedIn' },
        { platform: 'twitter', url: 'https://twitter.com/user3', display_text: 'Twitter' },
      ];
      ...
      // Set up error on index 0 and auto-gen on index 2
      act(() => {
        result.current.handleSocialLinkChange(0, 'url', 'bad-url');
        result.current.handleSocialLinkChange(2, 'url', 'https://twitter.com/user3');
      });
      ...
      // Remove link at index 1 (middle)
      act(() => {
        result.current.handleRemoveSocialLink(1);
      });

      // Verify:
      // - Error on index 0 stays at index 0 (before removed)
      // - Auto-gen on index 2 moves to index 1 (after removed, shifted down)
      expect(result.current.socialLinkErrors[0]).toBe('Invalid URL for platform');
      expect(result.current.autoGeneratedIndexes.has(0)).toBe(false);
      expect(result.current.autoGeneratedIndexes.has(1)).toBe(true);
    });
