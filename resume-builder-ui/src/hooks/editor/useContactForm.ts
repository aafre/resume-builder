...
export const useContactForm = ({
   ...
   *
   * IMPORTANT: Remaps indices for all items after the removed one, since
   * array removal causes subsequent items to shift down by one index.
   *
  ...
  const handleRemoveSocialLink = useCallback(
    (index: number) => {
      ...
      // Clear error for removed index and shift subsequent indices down by one
      setSocialLinkErrors((prev) => {
        const updated: Record<number, string> = {};
        Object.keys(prev).forEach((key) => {
          const i = parseInt(key, 10);
          if (i < index) {
            // Keep items before removed index as-is
            updated[i] = prev[i];
          } else if (i > index) {
            // Shift items after removed index down by one
            updated[i - 1] = prev[i];
          }
          // Skip i === index (removed item)
        });
        return updated;
      });

      // Remove from auto-generated indexes and shift subsequent indices
      ...
      // Clear pending debounce for removed index and shift refs
