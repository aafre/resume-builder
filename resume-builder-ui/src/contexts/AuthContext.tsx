...
// --- Pure helper functions for legacy resume migration ---
// Defined outside the component to avoid stale closures in useEffect.
...
/** Find all legacy localStorage keys from main branch (pattern: resume-builder-{templateId}-autosave) */
function findAllLegacyResumes(): FoundLegacyResume[] {
  try {
...
/** Migrate multiple legacy resumes to cloud */
async function migrateAllLegacyResumes(session: Session, legacyResumes: FoundLegacyResume[]): Promise<boolean> {
  try {
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
        ...
        (event, session) => {
          ...
          if (event === 'SIGNED_IN' && session?.user && !session.user.is_anonymous) {
            ...
            if (provider === 'google' || provider === 'linkedin_oidc' || provider === 'email') {
              // A user upgrading from anonymous → identified is a new signup.
              // An existing identified user logging back in has no anonymous migration.
              const isNewUser = !!oldAnonUserId && oldAnonUserId !== session.user.id;
              trackSignedIn({
  ...
  const signOut = async () => {
    ...
    try {
      ...
      // Reset analytics identity (disconnect session from signed-in user)
      resetUser();
