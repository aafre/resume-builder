...
export function GhostCard({ isAtLimit, resumeCount, onCreateNew }: GhostCardProps) {
  ...
  if (isAtLimit) {
    ...
    if (isOverLimit) {
      return (
        <div className="h-full min-h-[320px] bg-gradient-to-br from-red-50 via-orange-50 to-red-50 border-2 border-red-400 rounded-lg p-6 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-stone-warm mb-4 text-center max-w-xs">
    ...
    return (
      <div
        className="h-full min-h-[320px] bg-chalk border-2 border-mist rounded-lg p-6 flex flex-col items-center justify-center"
      >
        ...
        <p className="text-sm text-stone-warm font-medium text-center">
  ...
  return (
    ...
    >
      <PlusCircle className="w-16 h-16 text-mist mb-4 group-hover:text-accent-text group-hover:scale-110 transition-all duration-200" />
