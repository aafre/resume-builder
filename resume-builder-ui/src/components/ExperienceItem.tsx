...
export const ExperienceItem = memo(({
  ...
  const handleUpdateField = (field: keyof ExperienceItemData, value: ExperienceItemData[keyof ExperienceItemData]) => {
  ...
  const handleDescriptionUpdate = (descIndex: number, value: string) => {
    ...
    onUpdate(index, { ...item, description: updatedDescriptions });
  };
  ...
  const sectionIdBase = `experience-${sectionName.replace(/\s+/g, '-').toLowerCase()}`;
          ...
          <div className="space-y-3 mt-2">
            {item.description.length > 0 && (
              <ItemDndContext
                ...
                sectionId={`${sectionIdBase}-item-${index}`}
                isSubitem={true}
