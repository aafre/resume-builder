...
// Define ExperienceItem interface locally to match ExperienceSection usage
...
interface ExperienceItemProps {
  ...
  index: number;
  sectionId: string; // Used for generating unique IDs for DnD
  onUpdate: (index: number, updatedExperience: ExperienceItem) => void;
...
const ExperienceItemComponent: React.FC<ExperienceItemProps> = ({
  ...
  // Stable handler for field updates
  const handleFieldChange = useCallback(
    (field: keyof ExperienceItem, value: ExperienceItem[keyof ExperienceItem]) => {
      onUpdate(index, {
        ...experience,
        [field]: value,
      });
  ...
  // Stable handler for icon changes
  const handleIconChange = useCallback(
    (filename: string | null, file: File | null) => {
      onUpdate(index, {
        ...experience,
        icon: filename,
        iconFile: file,
  ...
  // Stable handler for description updates
  const handleDescriptionChange = useCallback(
    (descIndex: number, value: string) => {
  ...
  // Stable handler for removing description
  ...
  // Stable handler for adding description
  ...
  // Stable handler for reordering descriptions
  const handleReorderDescription = useCallback(
    (oldDescIndex: number, newDescIndex: number) => {
      const newDescription = arrayMove(
                        ...
                        <div className="flex items-start gap-3">
                          <div className="flex-1">
                            <RichTextInput
                              value={desc}
                              onChange={(value) =>
                                handleDescriptionChange(descIndex, value)
                              }
