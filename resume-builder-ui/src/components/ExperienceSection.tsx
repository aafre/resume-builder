...
import ExperienceItem, { EditorExperienceItem, IconRegistryMethods } from "./ExperienceItem";

interface ExperienceSectionProps {
  sectionName: string; // Custom section title
  experiences: EditorExperienceItem[];
  onUpdate: (updatedExperiences: EditorExperienceItem[]) => void;
...
const ExperienceSection: React.FC<ExperienceSectionProps> = ({
  ...
  // Ref to hold the latest experiences for stable callbacks
  // This prevents the callback identity from changing when the list updates
  const experiencesRef = useRef(experiences);
  experiencesRef.current = experiences;

  // Stable handler for item updates
  const handleUpdateItem = useCallback((index: number, updatedItem: EditorExperienceItem) => {
