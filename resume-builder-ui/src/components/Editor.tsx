...
const Editor: React.FC = () => {
  ...
  const handleGenerateResume = async () => {
    try {
      // Validate LinkedIn URL before generating resume
      if (!contactInfo?.linkedin || !validateLinkedInUrl(contactInfo.linkedin)) {
        toast.error("Please enter a valid LinkedIn URL before generating your resume");
        return;
      }
  ...
  const generateLinkedInDisplayText = useCallback(async (linkedinUrl: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Only auto-populate if field is empty or contains previous suggestion
        const currentDisplayText = contactInfo.linkedin_display || "";
        if (
          !currentDisplayText ||
          currentDisplayText === lastLinkedInSuggestion
        ) {
  ...
  const validateLinkedInUrl = (url: string): boolean => {
    if (!url.trim()) {
      ...
      return false; // Now mandatory field
    }
  ...
  // Validate LinkedIn field on component load to show required field error
  useEffect(() => {
