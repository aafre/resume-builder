...
const Editor: React.FC = () => {
  ...
  // Handle AI-imported resume from TemplateCarousel
  useEffect(() => {
    ...
    if (importedParam === 'true') {
      ...
      if (yamlString) {
        try {
          ...
          // Import icons if present
          if (parsedYaml.__icons__ && Object.keys(parsedYaml.__icons__).length > 0) {
            iconRegistry.importIconsFromYAML(parsedYaml.__icons__).catch(console.error);
          }
          ...
          setAIWarnings(warningsStr ? JSON.parse(warningsStr) : []);
          setAIConfidence(confidenceStr ? parseFloat(confidenceStr) : 0);
