...
describe('TemplateSelectionModal', () => {
  ...
  it('calls onClose when backdrop is clicked', async () => {
    ...
    // ModalShell dismisses on mousedown, not click, so a drag that starts
    // inside the panel and releases over the backdrop does not close it.
    fireEvent.mouseDown(screen.getByTestId('template-selection-modal-backdrop'));
    expect(onClose).toHaveBeenCalledTimes(1);
  });
  ...
  it('has accessible dialog role and aria attributes', async () => {
    ...
    // Heading id is generated by useId now (a hard-coded id would collide if
      // two dialogs mounted together), so assert the resolved accessible name.
      expect(dialog).toHaveAccessibleName('Choose Your Style');
  });
