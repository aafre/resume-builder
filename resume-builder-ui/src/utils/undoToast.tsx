...
/**
 * The editor has no history stack and no Ctrl+Z. This 8-second
 ...
 */
// 8s: long enough to notice a mis-tap and reach the button. The window also
// freezes while the toast has focus (WCAG 2.2.1), not only on hover, which is
// all react-hot-toast pauses for on its own.
export const UNDO_TOAST_MS = 8000;

function UndoToastBody({ id, message, onUndo }: { id: string; message: string; onUndo: () => void }) {
  const { handlers } = useToaster();
  return (
    <span className="flex items-center gap-3" onFocus={handlers.startPause} onBlur={handlers.endPause}>
      <span>{message}</span>
      <button
        type="button"
        onClick={() => {
        ...
        className="-my-2 inline-flex min-h-11 flex-shrink-0 items-center rounded-lg border border-white/30 px-3 font-semibold text-white transition-colors duration-150 hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
      >
        Undo
      </button>
...
export function toastUndo(message: string, onUndo: () => void): string {
  return toast((t) => <UndoToastBody id={t.id} message={message} onUndo={onUndo} />, {
