...
def _download_icon_worker(session_icons_dir, icon):
    """
    Worker function to download a single icon in a thread pool.
    """
    icon_path = session_icons_dir / icon["filename"]
    success = download_icon_from_storage(icon["storage_path"], str(icon_path))
    return icon["filename"], success
...
def generate_pdf_for_saved_resume(resume_id):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            # Download icons from storage concurrently
            failed_icons = []
            if icons_result.data:
                with ThreadPoolExecutor(max_workers=MAX_ICON_COPY_WORKERS) as executor:
                    worker = partial(_download_icon_worker, session_icons_dir)
                    results = executor.map(worker, icons_result.data)
                    for filename, success in results:
                        if not success:
                            failed_icons.append(filename)
                            logging.error(f"Failed to download icon: {filename}")
