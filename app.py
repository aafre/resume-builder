...
# ⚡ Bolt Optimization: Pre-compile LaTeX escape pattern at module level
_LATEX_ESCAPE_PATTERN = re.compile(
    "|".join(re.escape(key) for key in _LATEX_SPECIAL_CHARS.keys())
)
...
def _escape_latex(text):
    ...
    escaped_text = _LATEX_ESCAPE_PATTERN.sub(
        lambda match: _LATEX_SPECIAL_CHARS[match.group(0)], text
    )
    return escaped_text
...
# ⚡ Bolt Optimization: Pre-compile markdown link pattern at module level
_COMPILED_MARKDOWN_LINK_PATTERN = re.compile(_MARKDOWN_LINK_PATTERN)
...
def _convert_markdown_links(text, replacement):
    ...
    if not text or not isinstance(text, str):
        return text
    return _COMPILED_MARKDOWN_LINK_PATTERN.sub(replacement, text)
...
# ⚡ Bolt Optimization: Pre-compile markdown formatting patterns at module level
_COMPILED_MARKDOWN_FORMATTING_RULES = [
    (re.compile(pattern), r1, r2) for pattern, r1, r2 in _MARKDOWN_FORMATTING_RULES
]
...
def _apply_markdown_formatting(text, rule_index):
    ...
    if not text or not isinstance(text, str):
        return text
    for pattern, r1, r2 in _COMPILED_MARKDOWN_FORMATTING_RULES:
        replacements = (r1, r2)
...
# ⚡ Bolt Optimization: Pre-compile stray escaping patterns at module level
_STRAY_UNDERSCORE_PATTERN = re.compile(r"(?<!\\)_")
...
def _escape_remaining_latex_chars(text):
    ...
    text = _STRAY_UNDERSCORE_PATTERN.sub(r"\\_", text)

    # Escape tildes NOT already escaped
    text = _STRAY_TILDE_PATTERN.sub(r"\\textasciitilde{}", text)
