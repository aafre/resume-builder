...
_COMPILED_MARKDOWN_FORMATTING_RULES = [
    (re.compile(pattern), html, latex)
    for pattern, html, latex in _MARKDOWN_FORMATTING_RULES
]
...
def _apply_markdown_formatting(text, rule_index):
    ...
    if not text or not isinstance(text, str):
        return text
    for pattern, html, latex in _COMPILED_MARKDOWN_FORMATTING_RULES:
        text = pattern.sub(html if rule_index == 1 else latex, text)
    return text
...
_ESCAPE_UNDERSCORE_PATTERN = re.compile(r"(?<!\\)_")
_ESCAPE_TILDE_PATTERN = re.compile(r"(?<!\\)~")
...
def _escape_remaining_latex_chars(text):
    ...
    text = _ESCAPE_UNDERSCORE_PATTERN.sub(r"\\_", text)

    # Escape tildes NOT already escaped
    text = _ESCAPE_TILDE_PATTERN.sub(r"\\textasciitilde{}", text)
