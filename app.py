...
_LATEX_ESCAPE_UNDERSCORE = re.compile(r"(?<!\\)_")
_LATEX_ESCAPE_TILDE = re.compile(r"(?<!\\)~")
...
def _escape_remaining_latex_chars(text):
    ...
    text = _LATEX_ESCAPE_UNDERSCORE.sub(r"\\_", text)

    # Escape tildes NOT already escaped
    text = _LATEX_ESCAPE_TILDE.sub(r"\\textasciitilde{}", text)
