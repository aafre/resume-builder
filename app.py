...
_LATEX_SPECIAL_CHARS_PATTERN = re.compile(
...
def _escape_latex(text):
    ...
    escaped_text = _LATEX_SPECIAL_CHARS_PATTERN.sub(
...
def _convert_markdown_links(text, replacement):
    ...
    if not text or not isinstance(text, str):
        return text
    return _MARKDOWN_LINK_PATTERN_COMPILED.sub(replacement, text)
...
_ESCAPED_TILDE_PATTERN = re.compile(r"(?<!\\)~")
...
def _escape_remaining_latex_chars(text):
    ...
    text = _ESCAPED_UNDERSCORE_PATTERN.sub(r"\\_", text)

    # Escape tildes NOT already escaped
    text = _ESCAPED_TILDE_PATTERN.sub(r"\\textasciitilde{}", text)
