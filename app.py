...
def load_resume(resume_id):
    ...
    try:
        ...
        try:
            supabase.table("resumes").update({"last_accessed_at": "now()"}).eq(
                "id", resume_id
            ).execute()
...
def migrate_anonymous_resumes():
    ...
    try:
        ...
        supabase.table("resumes").update({"user_id": new_user_id}).in_(
            "id", old_resume_ids
        ).execute()
...
def update_resume_partial(resume_id):
    ...
    try:
        ...
        return (
            jsonify({"success": True, "title": new_title, "updated_at": updated_at}),
            200,
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            with open(yaml_path, "w") as f:
                fast_yaml_dump(
                    yaml_data, f
                )  # Bolt: Use fast_yaml_dump to speed up YAML serialization
